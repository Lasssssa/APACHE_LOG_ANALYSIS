...
if [ "$1" = "" ]; then
  echo "No directory given, defaulting to the current directory."
  Directory="."
else
  echo "| Test id : $1"
  if [ -d "$1" ]; then
...
resultRC=2
if [ -r "returncode" ]; then
  if [ "$returnCode" = "$(cat returncode)" ]; then
...
if ls *.outfile &> /dev/null; then
  ...
  if [ $resultFiles -eq 2 ]; then
    resultFiles=1
  fi
fi

echo "                                       --------------------"
if [ $resultGlobal -eq 0 ]; then
  echo "                                       Global      : FAILED"
else
  echo "                                       Global      : PASSED"
fi
